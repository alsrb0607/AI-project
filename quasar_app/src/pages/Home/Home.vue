  ...
  <page>
    ...
    <page-body>
      <div class="q-pa-xs q-gutter-md row items-start " style="background-color: #fbfbfb;">
        <!-- 상단 -->
        <q-card class="top-card " flat style="margin-left: 20px" >
    ...
    <div class="text-weight-medium  text-h6" style="font-weight: 500; margin-left: 20px ;background-color: #fbfbfb;">
        <span :style="themeColorLine2">&#124;</span> {{ $t('home_service') }}
    </div>
    <div class="q-pa-md row items-start q-gutter-sm justify-between" style="margin-left: 0px">
        ...
        <q-card-section horizontal>
          <q-card-section>
            <q-img style="width: 75px" src="~assets/icons/chest2.png" /> 
            <br>
            <span 
            class="flex-center row text-weight-bold text-size-16">{{ $t('home_subtitle_chest') }}</span>
            </q-card-section>
            ...
            style="margin-left: 20px width:150px; height:25px"> <br>{{ $t('home_btn_waiting') }}: {{num_chest}}</span>
            <span 
            class="text-weight-medium text-size-16"
...
export default {
  ...
  setup () {
    ...
    let themeColorLine = ref("")
    let themeColorLine2 = ref("")
    function goXray() {
      $router.push('/xrays')
    }
    ...
    const profile = "/images/profile/profile2.png"
    ...
    onActivated(() => {
      getNum()
      // 문자 안에 formating 하기 위해서 `` 사용(1 옆에)
      themeColorLine.value = `background: linear-gradient(to top, ${getCssVar('primary')} 10%, transparent 40%)`
      themeColorLine2.value = `color: ${getCssVar('primary')}; width: 5px`
