      ...
      >
        ...
        >
          ...
          <div
            ...
            class="row justify-center"
          >
            <div class="text-h6 q-mb-md">{{ $t("detect_diagnosis") }}: {{ $t(data.name) }} <br/>
            {{ $t("detect_age") }}: {{ image.age }} <br/>{{ $t("detect_date_xray") }}: {{ image.date }}</div>
            <div 
            v-if="data.img_url"
            class="q-mb-md">
            <q-img
...
export default {
  ...
  setup() {
    ...
    async function getValues() {
     ...
     console.log('데이터 잘 가져옴', image.value, imageId.value, bookmarks.value, detect.value, text.value)}
    }
